...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    exclude-result-prefixes="xalan number aggregates date string sql double duration"
    version="1.0">
    ...
    <xsl:template match="UNB" mode="SUMSHEET">
        ...
        <xsl:element name="OTHER">	 
	        <xsl:attribute name="T">
	             <xsl:value-of select="number:getValue('totalOthers')"/>
	         </xsl:attribute> 	                          	            			
		    <xsl:for-each select="LIN1/OCC">
	    		<xsl:if test="not (starts-with(@DS,'Pagamento em duplicidade'))
	    			...
	    			<xsl:variable name="null" select="number:setValue('totalOthers', number:getValue('totalOthers') + double:sum(LIN1/OCC/@V))"/> 	    
        ...
        <xsl:element name="CREDITS">            
	        <xsl:attribute name="T">
	             <xsl:value-of select="number:getValue('totalCredits')"/>
	         </xsl:attribute> 	  
    		<xsl:for-each select="LIN1/OCC">
        ...
        <xsl:element name="PENALTY_INTERESTS">           
	        <xsl:attribute name="T">
	             <xsl:value-of select="number:getValue('totalPenaltyInterests')"/>
	         </xsl:attribute> 	                         
    		<xsl:for-each select="LIN1/OCC">
